...
int stop_mode_packet(char *q)
{
	...
	q[8] = encrpty_cpuid[3];
	q[9] = encrpty_cpuid[4];
	q[10] = encrpty_cpuid[5];
	q[11] = encrpty_cpuid[6];
	q[12] = encrpty_cpuid[7];
	q[13] = encrpty_cpuid[8];
	q[14] = encrpty_cpuid[9];
	q[15] = encrpty_cpuid[10];
	q[16] = encrpty_cpuid[11];
	q[17] = encrpty_cpuid[12];
	q[18] = encrpty_cpuid[13];
	q[19] = encrpty_cpuid[14];
	q[20] = encrpty_cpuid[15];
	q[21] = encrpty_cpuid[16];
	q[22] = encrpty_cpuid[17];
...
int decode_packet_robot( packet_robot_t *packet, unsigned char *data, int len )
{
	...
	//处理回包信息
	// if(data[pos+22]!=255){
	// 	infrared = (data[pos+22] >> 6)%2==1;
	// 	flat = (data[pos+22] >> 5)%2==1;
	// 	chip = (data[pos+22] >> 4)%2==1;
	// }
	return 0;
