...
u8  encrpty_cpuid[18];//加密前原文前10字节 + 加密后的cpuid  8字节
...
* Return		  : None.
*******************************************************************************/
void calcute_encrpty_cpuid(u8 bat_v)
{
	...
	encryptData(&encrpty_cpuid[0], &cpuid_data[0], 12, bat_v);

	for(i=0;i<18;i++)
	{
	
		if(encrpty_cpuid[i] == 0xFF)
		{
			encrpty_cpuid[i] = 0xFE;
		}
...
{   
	...
	switch(g_robot.mode)
	{
	  ...
	  case BlUETOOTH_MODE : //蓝牙模式也加入陀螺仪
