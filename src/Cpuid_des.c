...
* V1.1		tianqinglin 2016.12.23  修改文件
*****************************************************************************
*****************************************************************************/
...
/*
 * 提供CPUID加密算法
 * dest - 保存加密后结果，空间要求至少8字节
 * org  - 传入原始数据
 * len  - 传入的原始数据的长度
 */
void encryptCPUID(uint8_t *dest, const uint8_t *org, int len)
{
	uint8_t o1[16];
	uint8_t key[16];
	uint8_t p1[16];
...
/*
* 提供数据加密算法
* dest - 保存加密后原文+密文，空间要求至少18字节
* org  - 传入原始数据
* len  - 传入的原始数据的长度，要求超过10字节
* key_ch - 传入密钥一部分
*/
int encryptData(uint8_t *dest, const uint8_t *org, int len, uint8_t key_ch)
{
	uint8_t o1[16];
	uint8_t key[16];
	uint8_t p1[16];
	...
	ZERO_STRUCT(p1);
	memcpy(o1, org, 8);
	memcpy(key, "hznjrobot", 5);
	memcpy(key+5, org+8, 2);
	key[org[0] & 0x03] = key_ch;
	des_crypt56(p1, o1, key, 1);
	p1[org[0] & 0x07] = key_ch;
	memcpy(dest, org, 10);
	memcpy(dest+10, p1, 8);
	return 1;
}
...
* 提供验证加密数据
* org  - 传入待验证数据
* len  - 传入的原始数据的长度，要求超过18字节
*/
int verifyData(const uint8_t *org, int len)
{
	uint8_t dest[24];
	if (len < 18)
		return 0;
	if (!encryptData(dest, org, len, org[10 + (org[0] & 0x07)]))
		return 0;
	return (memcmp(dest, org, 18) == 0);
}
