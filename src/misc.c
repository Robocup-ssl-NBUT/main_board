...
void init_shoot_chip_timer(void)
{
	...
	prescale = (u16) (FREQ_PCLK_2 * 2 / SHOOT_TIM_CLK_FRQ) - 1;//10Khz周期

	/* Time base configuration */
	TIM_TimeBaseStructure.TIM_Period = MAX_SHOT_STRENGTH + 2;  //周期为150/10k=15ms
	...
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;   //chip高电平有效
	TIM_OC2Init(TIM9, &TIM_OCInitStructure);

	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2;
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;  //SHOOT 低电平有效
...
void init_ir_pwm(void)
{
	...
	/* Time base configuration */
	TIM_TimeBaseStructure.TIM_Period = pwm_period;    //周期1/38K
	TIM_TimeBaseStructure.TIM_Prescaler = prescale;
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	...
	/* PWM1 Mode configuration: Channel1 */
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1; //TIM_CNT < TIM_CCR1 通道为有效状态
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
	TIM_OCInitStructure.TIM_Pulse = SystemCoreClock / IR_BALL_PWM_FREQ * IR_BALL_PWM_DUTY / 100;
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;  //高有效 0.2/38K 高电平为有效状态
