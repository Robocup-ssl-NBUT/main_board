...
vu8 bluetooth_mode_on = 0; //蓝牙模式标志
...
timer_t bluetooth_mon_timer;  //蓝牙监控时间设置为25ms
...
extern int AT_Set_device_name_code(char *code);
extern int AT_Set_device_name_cmd(char *name);
...
int init_robot(void)
{
	...
	load_param(&param);
	wheel_reduction_ratio_x_set = param.dat[13];
 	wheel_reduction_ratio_yz_set = param.dat[14];
    eeprom_blue_init_flag = param.dat[16];
	if(param.dat[15] <= MAX_SHOT_STRENGTH)
    {
    	max_shot_strength_set = param.dat[15];
    }
    else 
    {
...
int do_power_monitor(void)
{
	...
	{ 
		...
		else
		{
			...
			else if((beepcnt > 0) && (beepcnt < 20) && (beepflag == 1))
			{
				BEEP_ON();
				beepcnt--;
			}
			...
			else if((beepcnt > 0) && (beepcnt < 20) && (beepflag == 0))
			{
				BEEP_OFF();
				beepcnt++;
			}			
...
void do_run(void)
{
	...
	gyro_mon_timer = get_one_timer(GYRO_MON_TIME);  //陀螺仪控制周期
	bluetooth_mon_timer = get_one_timer(BLUETOOTH_MON_TIME);  //蓝牙数据通信监控周期
	/* start motor */
	start_motor();
	...
	{	   	
		...
		{ 
			...
			case NORMAL_MODE :
			{
				...
				if(bluetooth_mode_on == 1) //蓝牙模式
				{
				   if(check_timer(bluetooth_mon_timer) == 0)//蓝牙没有超时
				   {
				...
				if(bluetooth_mode_on == 0)//非蓝牙模式即比赛模式需要身份认证
				{
			...
			{ 
				...
				set_heart_led(100, 100, 100, 0x1f);//白色重新设置一次		
			...
			{	
				...
				do_dribbler(0);
				wait_ms(500);
				set_shooter( SHOOT, 10 );//平射
		        wait_ms(2500);
				set_shooter( CHIP, 10 ); //挑射
            	wait_ms(2000);
