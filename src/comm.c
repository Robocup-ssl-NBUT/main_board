...
extern vu8 bluetooth_mode_on;
extern timer_t bluetooth_mon_timer;
...
int do_comm(void)
{
	...
	{   
		...
		{   
		   ...
		   {
		      if(get_uart_ble_packet(&buf_packet_ok[0]) == 0) //有蓝牙数据包
		      {
				  ...
				  g_rf_comm.buffer_pos = UART_PCKT_RX_SIZE;
				   ...
				   bluetooth_mon_timer = get_one_timer(BLUETOOTH_MON_TIME);
			  }
...
int do_comm_up(void)
{
	...
	if(type == PACKET_Normal)
	{
		...
		if( (((g_rf_comm.buffer[3] & 0x8) >> 3) == 1) || (((g_rf_comm.buffer[9] & 0x8) >> 3) == 1) 
			|| (((g_rf_comm.buffer[15] & 0x8) >> 3) == 1)) //bit3 为1 则上传模式为2上传电池电容电压
		{
			...
			if( (temp != -1) && (((g_rf_comm.buffer[3+6*temp] & 0x8) >> 3) == 1) )
			{
				...
				{  
					...
					nRF24L01_dev.buf.buf[15]=data[15];
					nRF24L01_dev.buf.buf[16]=data[16];
					nRF24L01_dev.buf.buf[17]=data[17];
					nRF24L01_dev.buf.buf[18]=data[18];
					nRF24L01_dev.buf.buf[19]=data[19];
					nRF24L01_dev.buf.buf[20]=data[20]; 
					nRF24L01_dev.buf.buf[21]=data[21];
					nRF24L01_dev.buf.buf[22]=data[22];
					memset(&nRF24L01_dev.buf.buf[23], 0, sizeof(unsigned char) * (PACKET_LEN_UP - 23));
...
{   
	...
	if(bluetooth_mode_on == 0)//正常比赛模式需要身份认证
	{
    ...
    /*----------------------------通常模式下的数据解包--------------------------------*/
	/*-----------------------参见协议0.1比赛时通讯包格式-----------------------------*/
	if(((type == PACKET_Normal) && (g_robot.mode == NORMAL_MODE)) || (g_robot.mode == BlUETOOTH_MODE))
	{
		/* parse robot command */
		...
		#if 0
		if(g_robot.mode == BlUETOOTH_MODE)
		{
		    if((packet.speed_rot == 0) && ((packet.speed_x != 0) || (packet.speed_y != 0)))
		    {
		       gyro_control_flag = 1;  //小车xy向右速度值且旋转方向没有速度值则进行陀螺仪闭环控制
               packet.speed_rot = (int)(gyro_pid.out);
		    }
			else
		    {
		        gyro_control_flag = 0;
