...
CDC_IF_Prop_TypeDef VCP_fops = 
{
  ...
  VCP_DeInit,
  VCP_Ctrl,
  VCP_DataTx,     //处理要发送的数据通过IN端点发送给PC
  VCP_DataRx      //处理从usb out端点接收到的数据
};
...
/**
  ...
  * @retval Result of the opeartion: USBD_OK if all operations are OK else VCP_FAIL
  从串口接收数据 APP_Rx_Buffer数组数据发送给PC机
...
/**
  ...
  * @retval Result of the opeartion: USBD_OK if all operations are OK else VCP_FAIL
  从usb OUT端点接收到的数据函数处理
  usb out 端点中断函数调用,接收到从主机发来的数据
  */
static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
  uint32_t i;
